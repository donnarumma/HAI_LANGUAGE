...
- `TEST_HAI_UNKNOWN.m`: reusable novel-word simulation called by the batch
  workflow. It saves each generated dictionary as
  `BERT/BERT_v1_<NEWWORD>/BERT_v1_<NEWWORD>.m`; for example, passing
  `ANOTHER` creates `BERT_v1_ANOTHER`.

Two generated dictionary snapshots are versioned with explicit names:
`BERT_v1_BUTTER`, used by the `s03`, `s04`, and `s06` demonstrations, and
`BERT_v1_ANOTHER`, generated by `TEST_HAI_UNKNOWN`. Other intermediate
dictionaries and `.mat` files, including content under
`DICTIONARY/BERT_DIC/`, are not versioned. Run `s01` to generate the starting
BERT state before scripts that load `BERT_v1_S01/MDP_STEP001.mat`. The batch
`s05` workflow also expects a generated `DICTIONARY/word_new.mat` word list.
