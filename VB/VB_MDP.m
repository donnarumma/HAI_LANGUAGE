...
%--------------------------------------------------------------------------
if isfield(MDP,'U'), OPTIONS.level(MDP.level).gamma = 1;         end
...
for t = 1:MDP.T
    ...
    if OPTIONS.debugmode
        try 
            fprintf('Level%g,Time %g\n',MDP.level,t)
            % MDP.s(:,t)
            % HAI_retrieveLevel(OPTIONS.level(MDP.level).STATES{MDP.s(1,t)})
            if ismember(t,OPTIONS.tStop)
                pause;
            end
        catch
        end
    end   
    try % remove unknown content 
        for iLev=1:length(OPTIONS.level)
            if ~isempty(OPTIONS.level(iLev).idkContent) && MDP.level==iLev
                indW=OPTIONS.level(iLev).idkContent;
                if t==1
                    MDP.D{1}(indW)=0;
                else
                    MDP.X{1}(indW,:)=0;
                    for ix=1:t
                        MDP.X{1}(:,ix)=MDP.X{1}(:,ix)/sum(MDP.X{1}(:,ix));
                    end
                end
                fprintf('removed content\n');
            end
        end
    catch
    end
    ...
    for istatef = 1:NstateFactors
        ...
        if MDP.s(istatef,t) == 0
            if OPTIONS.level(MDP.level).sX(1) && MDP.s(istatef,t)==0 % DONNARUMMA Sx if you do not set up s, then s becomes best state guess
    ...
    if isfield(MDP,'link')
        ...
        try
            ...
            mdp = MDP.MDP(1);
            if OPTIONS.level(MDP.level).sX(2)
                try
                    mdp.t=[{0} , MDP.t(:)'];
    ...
    if OPTIONS.level(MDP.level).gamma
        MDP.w(t) = 1/MDP.beta;
    else
    ...
    try
        for iLev=1:length(OPTIONS.level)
            if ~isempty(OPTIONS.level(iLev).idkContent) && MDP.level==iLev
                ...
                MDP.Q{1}(indW,:)=0;
                for ix=1:t
                    MDP.Q{1}(:,ix)=MDP.Q{1}(:,ix)/sum(MDP.Q{1}(:,ix));
                end
                fprintf('removed content\n');
            end
        end
    ...
    if isfield(MDP,'factor') 
        ...
        for istatef = MDP.factor(:)'
            ...
            qx     = MDP.X{istatef}(:,Ht);
%             qx     = MDP.X{m,istatef}(:,end); % take last 
            UNCERTAINTY(istatef) = qx'*spm_log(qx);
        end
