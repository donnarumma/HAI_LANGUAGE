...
% params.VARIABLES_DYSLEXIA               = str2func(dictionary);                                                       
params.DICTIONARY                       = str2func(dictionary);                                                       
% params.VARIABLES_DYSLEXIA             = str2func('VARIABLES_DYSLEXIA_v1');
% DICTIONARY                              = params.VARIABLES_DYSLEXIA();
...
for iLev=1:NLevels
    ...
    params.level(iLev).maxT             = 20;               % max time depth per level
    params.level(iLev).create           = str2func('HAI_createLevel');
    params.level(iLev).MDP              = [];               % initialize empty sublevel
    params.level(iLev).jump             = 1;                % 1 location can be reached from any state | 0 sequential location steps
    params.level(iLev).umode            =-1;  % -1 all possible policies, 0 remove policy [1,1,1]
    params.level(iLev).chi              = 1/64;
...
% params.spm_MDP_VB_H                  =str2func('spm_MDP_VB_X_tutorial_debug_v2');       % Friston simulation
% params.spm_MDP_VB_H                  =str2func('spm_MDP_VB_X_tutorial_debug_v2_bis');   % Friston simulation with friendly variable
% params.spm_MDP_VB_H                  = str2func('spm_MDP_VB_X_tutorial_debug_v5');       % Latest version
% params.spm_MDP_VB_H                  = str2func('spm_MDP_VB_X_tutorial_debug_v7');       % Latest version
% params.spm_MDP_VB_H                  = str2func('spm_MDP_VB_X_tutorial_debug_v8');       % Latest version
