function   MDP=HAI_buildLevel(params)
% function MDP=HAI_buildLevel(params)
...
A       = params.getA(OBS,STATES,CLASSES,length(LOCATIONS),params);%oval,maskval,params.unknown);

%% Level Transition matrix p(St+1|St)
% B     = HAI_getB(Ns,sval,params.jump,params.unknown);
B       = params.getB(Ns,params);
%--------------------------------------------------------------------------
...
% --------------------------------------------------------------------------
% spm_MDP_check build all possible next policies
%
...
mdp.chi     = params.chi;
% if params.chi
    mdp.tau     = params.tau;  % high values -> slow convergence but more stable
% end
mdp.sname{1}= STATES;
...
if isempty(params.C)
    for iFactor = 1:length(A)
        mdp.C{iFactor}=zeros(size(A{iFactor},1),1);
    end
else
    mdp.C = params.C;           % preferences
end
