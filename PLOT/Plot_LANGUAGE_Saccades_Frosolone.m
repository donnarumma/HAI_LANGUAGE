function   hfig=Plot_LANGUAGE_Saccades_Frosolone(MDP,NSUB,GT)
...
% Creation of x_max and y_max limits
    ...
    for lin = 1:N_LINE
        ...
        for iw = 1+NUMBW_LINE*m:iw_end(lin)
            lenWs(len_ws)=length(retrieveLevel(phr{iw}));
            len_ws = len_ws + 1;
        end
...
set(cax,'color',0.95*[1 1 1])
    ...
    for p = 1: length(LINE_VEC)
        for iWW = 1:LINE_VEC(p)
            a = retrieveLevel(phr{iWW+NUMBW_LINE*m});
            for iLL = 1:numel(a)
                image(rx + y{1,p}(iWW,1) + x(iLL,1),ry + y{1,p}(iWW,2) + x(iLL,2),  createCharacterImage(a(iLL)));% hold on
            end
        end
        m = m+1;
